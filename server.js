...
============================================================
🛑 СТРОЖАЙШИЙ ЗАПРЕТ: НИКАКИХ СБОРЩИКОВ И ИХ СКЕЛЕТОВ
============================================================
Превью открывает СТАТИЧНЫЙ HTML напрямую, БЕЗ vite/webpack/next/parcel/CRA. Любой ответ, который опирается на сборщик, это БРАК — он откроется ПУСТОЙ страницей. Это типичная ошибка моделей, не повторяй её.

❌ ЗАПРЕЩЕНО ВЫВОДИТЬ ТАКОЕ (моментальный провал):
- \`<script type="module" src="/src/main.tsx"></script>\`
- \`<script type="module" src="/src/main.jsx"></script>\`
- \`<script type="module" src="/src/index.tsx"></script>\` или \`/src/index.jsx\`
- \`<script type="module" src="/main.tsx"></script>\` без приложенного исходника
- \`<link rel="icon" href="/vite.svg">\` (этого файла НЕТ)
- \`<link rel="stylesheet" href="/src/index.css">\`
- любые \`import ... from "/src/..."\` или \`import "..."\` из ЛОКАЛЬНЫХ путей
- Vite/CRA/Next/Nuxt/Parcel заголовки, манифесты, scaffold-файлы (\`vite.config.*\`, \`package.json\`, \`tsconfig\`, \`next.config\`).
- ссылки на локальные \`*.tsx\`, \`*.jsx\`, \`*.ts\`, \`*.module.css\`, которых нет в твоём ответе как полноценных \`\`\`file: блоков.

✅ ВМЕСТО ЭТОГО:
- Если задача без React — Tailwind+JS прямо в одном index.html (формат A).
- Если нужен React — встрой ВСЁ в один \`<script type="text/babel" data-presets="env,react">\` ВНУТРИ index.html (см. блок «React через CDN+Babel» выше). НИКАКИХ внешних .tsx/.jsx файлов и module-импортов.
- Иконку сайта либо опусти, либо встрой data-URL: \`<link rel="icon" href="data:image/svg+xml,..."/>\`.
- Все стили — tailwind-классами или внутри \`<style>\` в \`<head>\`.

ПРОВЕРЬ свой ответ перед отправкой:
1) Открой мысленно index.html в браузере КАК ЕСТЬ, без npm install, без сборки.
2) Если хоть одна ссылка \`src=\` / \`href=\` / \`import\` указывает на локальный файл, которого ты НЕ положил отдельным \`\`\`file: блоком — это БРАК. Перепиши.
...
app.post('/api/generate', async (req, res) => {
  try {
    ...
    let files = extractFiles(result.text);
    ...
    // АВТО-РЕТРАЙ: модель вернула scaffold или ссылки на отсутствующие файлы → даём
    // корректирующее указание и просим переписать самодостаточный standalone-проект.
    let retried = false;
    let retryReason = null;
    if (files && files.size > 0 && report && !report.ok) {
      retried = true;
      retryReason = describeIntegrity(report);
      const correctionMessages = [
        ...
        { role: 'assistant', content: result.text },
        {
          ...
          content: [
            ...
            'Если нужен React — встрой ВСЁ в один <script type="text/babel" data-presets="env,react"> прямо внутри index.html, без внешних tsx/jsx.',
      ...
      try {
        ...
        // Берём вторую попытку, если она строго лучше: либо валидна, либо имеет меньше missing-ссылок.
        const better = report2 && (
          report2.ok
          || (!report2.hasFrameworkScaffold && (!report || (report2.missing.length < (report?.missing?.length ?? 0))))
        );
        if (better && files2 && files2.size > 0) {
          result = r2;
          files = files2;
          report = report2;
    ...
    if (files && files.size > 0) {
      ...
      assistantText = result.text || 'Модель не вернула файлы. Попробуй переформулировать запрос или сменить модель.';
    }
    ...
    res.json({
      ...
      brokenProject: !!(report && !report.ok),
      missingFiles: report ? report.missing.map((m) => m.normalized || m.ref) : [],
      scaffoldKind: report?.scaffoldKind || null,
      retried,
